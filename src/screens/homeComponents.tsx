    ...
    <TabBarItem
      ...
      renderLabel={(scene: {route: Route; focused: boolean; color: string}) => {
        if (scene.route.key === 'glam') {
          return (
            <FastImage
              style={{width: 63, height: 26}}
              source={Img.icon.main.logo}
              tintColor={
        ...
        return (
          <Text style={{fontSize: 20, fontWeight: '600', color: scene.color}}>
            {props.route.title}
          </Text>
        );
      }}
...
export const ItemSeparatorComponent = useCallback(
  () => <View style={{height: 12}} />,
  [],
);

export const CustomRecommend = (props: {
  ...
  return (
    <View
      ...
      <CustomRecommendLine
        icon={Img.icon.recommendations.today}
        title={'글램 추천'}
        isHOT
        onPress={onChoose}
      />
      <CustomRecommendLine
        icon={Img.icon.recommendations.dia}
        title={'최상위 매력'}
        isHOT
        onPress={onChoose}
      />
      <CustomRecommendLine
        icon={Img.icon.recommendations.glamour}
        title={'볼륨감 있는 체형'}
        isHOT
        onPress={onChoose}
      />
      <CustomRecommendLine
        icon={Img.icon.recommendations.withpet}
        title={'반려 동물을 키우는'}
        onPress={onChoose}
      />
      <TouchableOpacity
...
export const CustomRecommendLine = ({
  icon,
  title,
  onPress,
  isHOT,
  ...
  const onPress_ = () => {
    Alert.alert(title, '최근 접속한 2명을 추천합니다.', [
      {text: '추천받기', onPress: () => onPress(title)},
      {text: '돌아가기'},
    ]);
  };
  return (
    <TouchableOpacity
      ...
      {isHOT && (
        <FastImage
          style={{width: 30, height: 13, marginLeft: 8}}
          source={Img.icon.recommendations.hot}
        />
      )}
        ...
        <LinearGradient
          ...
          <View style={{paddingHorizontal: 16}}>
            ...
            <View style={{flexDirection: 'row', alignItems: 'center'}}>
              ...
              <FastImage
                style={{width: 16, height: 17, marginLeft: 4}}
                source={Img.icon.main.info}
              />
            </View>
            {item.introduction ? (
